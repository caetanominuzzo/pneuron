...
namespace primeira.pNeuron.Core
{
    ...
    public class NeuralValue
    {
        ...
        /// Internal value/delta of a synapse or bias.
        /// </summary>
        /// <param name="neuron">Neuron parent or null if bias.</param>
        ...
        public void CalculateDelta(double Error, double Momentum, double LearningRate)
        {
            //Sets delta for the neuron (or bias when Neuron is null).
            m_delta = Momentum * m_delta + LearningRate * Error * (Neuron == null ? 1 : Neuron.Value);
        }
    ...
    public class Neuron : INeuron
    {
        ...
        public void Pulse()
        {
            ...
            if (OnNeuronPulse != null)
                OnNeuronPulse(this, FlowDirection.Forward);
#endif
            lock (this)
            {
...
#if RAISE_ONPULSE
            if (OnNeuronPulse != null)
                OnNeuronPulse(this, FlowDirection.Backward);
#endif
            this.OutputReady = 0;

            if (this.NeuronType == NeuronTypes.Output)
            {
                this.Error = (desiredResult - this.Value) * Util.DerivativeSigmoid(this.m_value); 
            }
            else
            {
                this.Error = (desiredResult) * Util.DerivativeSigmoid(this.m_value); 
            }
        ...
        public void ApplyLearning()
        {
            ...
            foreach (KeyValuePair<INeuron, NeuralValue> m in m_input)
                m.Value.ApplyLearning();

            m_bias.ApplyLearning();
        ...
        /// </summary>
        public void CalculateDelta(double Momentum, double LearningRate)
        {
            NeuralNetwork.Log(1, "Start Neuron {0} CalculateDelta.", this.ToString());

            foreach (Neuron n in this.Input.Keys)
            {
                this.Input[n].CalculateDelta(this.Error, Momentum, LearningRate);
            }

            //this.Bias.Delta += this.Error * this.Bias.Weight;
            this.Bias.CalculateDelta(this.Error, Momentum, LearningRate);
        ...
        public void ResetMemory()
        {
            ...
            //So the Memory Neuron acts as a second bias
            Value = 1;
        }
...
#if RAISE_ONPULSE
        internal delegate void OnNeuronPulseDelegate(Neuron sender, FlowDirection direction);
    ...
    public class NeuralNetwork : INeuralNetwork
    {
        ...
        private double DEFAULT_MOMENTUM = .9;
        
        private int INNER_TRAINING_TIMES = 1;
        ...
        internal double GlobalError
        {
            get
            {
                ...
                foreach (Neuron n in Neuron)
                {
                    ...
                    {
                        dGlobalTemp += (n.Error * n.Error) / 2.0;
                    }
                }
        ...
        /// </summary>
        /// <returns></returns>
        public Neuron AddNeuron(double V)
        {
            Neuron n = new Neuron(this);
            AddNeuron(n);
#if RAISE_ONPULSE
            n.OnNeuronPulse += new Neuron.OnNeuronPulseDelegate(n_OnNeuronPulse);
#endif
            return n;
        }

#if RAISE_ONPULSE
        private void n_OnNeuronPulse(Neuron sender, FlowDirection direction)
        {
            if (OnPulse != null)
                OnPulse(sender, direction);
        ...
        public void CalculateDelta()
        {
            ...
            foreach (Neuron n in this.Neuron)
                n.CalculateDelta(this.Momentum, this.LearningRate);
        }
        ...
        public void Train(double[][] input, double[][] output)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (dGlobalError < 0.000000000000000000000000001)
                        m_stopOnNextCycle = true;
                }
            }
            finally
        ...
        public double TrainSession(double[][] inputs, double[][] outputs, int iterations)
        {
            ...
            lock (this)
            {
                ...
                for (int i = 0; i < iterations; i++)
                {
                    ...
                    {
                        if (inputs[j][0] == -1) //Reset Memory Command
                        {
                            ResetMemory();
                        }
                        else
                    ...
                    m_lastCalculatedGlobalError = dErrorSum / inputs.Length;    
...
#if RAISE_ONPULSE
        public delegate void OnPulseDelegate(Neuron sender, FlowDirection direction);
    ...
    #region Enums NeuronTypes, DataTypes & FlowDirection
